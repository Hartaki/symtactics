...
let weatherGlitch = new Weather(
        "Weather Glitch",
        "All players may travel twice in a row.",
        { moves: 2 }
    ),
    heavyRain = new Weather(
        "Heavy Rain",
        "All players must travel at 1/2 of their normal speed.",
        { speedFactor: 0.5 }
    ),
    clear = new Weather("Clear", "Travel normally.", { speedFactor: 1 }),
    heatWave = new Weather(
        "Heat Wave",
        "Players may not ENTER or EXIT sites this round.",
        { enter_exit: false }
    ),
    pollutedWind = new Weather(
        "Polluted Wind",
        "All players must pay 2 resources if they wish to travel this round.",
        { resources: 2 }
    );
...
class Item {
    constructor(name, zone) {
        ...
        this.zone = zone;
        this.label =
            "Use this item to ignore the effects of events on " +
            zone.name +
            ". Add to discard pile after use.";
    }
}
...
let newspaper01 = new Newspaper("01", clear, bullMarket, {
    ...
    newspaper02 = new Newspaper("02", clear, bullMarket, {
        name: "Singapore Sniper!",
        description: "Friends and family say she was a straight shooter.",
        zone: hongKong,
    }),
    newspaper03 = new Newspaper("03", clear, normal, {
        name: "Tsunami",
        description: "A Tidal Wave of Concerns!",
        zone: hongKong,
    }),
    newspaper04 = new Newspaper("04", heatWave, pandemic, {
    ...
    newspaper05 = new Newspaper("05", heatWave, pandemic, {
        name: "SARS III",
        description: "",
        zone: mainland,
    }),
    newspaper06 = new Newspaper("06", heatWave, bioTech, {
        name: "SARS Mutation",
        description: "SAR-ry, there's still no cure!",
        zone: mainland,
    }),
    newspaper07 = new Newspaper("07", heatWave, techBubble, {
        name: "Drone Wars",
        description: "No end in sight. #boring",
        zone: islands,
    }),
    newspaper08 = new Newspaper("08", heavyRain, bioTech, {
        name: "VISA Shortage",
        description: "Sources credit banks",
        zone: mainland,
    }),
    newspaper09 = new Newspaper("09", heavyRain, normal, {
        name: "Cloning Scandal",
        description: "The same problems all over again.",
        zone: ocean,
    }),
    newspaper10 = new Newspaper("10", heavyRain, normal, {
        name: "Firmware Bugs!",
        description: "City calls in the SWAT team.",
        zone: mainland,
    }),
    newspaper11 = new Newspaper("11", heavyRain, bullMarket, {
        name: "Shamate Uprising",
        description: "City says shay-shay but no shay-shay to shamates.",
        zone: islands,
    }),
    newspaper12 = new Newspaper("12", heavyRain, bioTech, {
        name: "Hurrican Spime",
        description: "iEye did not see that coming!",
        zone: hongKong,
    }),
    newspaper13 = new Newspaper("13", pollutedWind, techBubble, {
        name: "SARS II",
        description: "",
        zone: hongKong,
    }),
    newspaper14 = new Newspaper("14", pollutedWind, bullMarket, {
        name: "Fisherman's STD",
        description: "Officials blame hookers",
        zone: ocean,
    }),
    newspaper15 = new Newspaper("15", pollutedWind, bullMarket, {
        name: "Driverless Traffic Jam",
        description: "Hong Kong?  More like HONK Kong!",
        zone: mainland,
    }),
    newspaper16 = new Newspaper("16", weatherGlitch, normal, {
        name: "Flood!",
        ...
        if (hex.players.length) {
            for (player of hex.players) {
                if (event.zone.name === hex.zone.name) {
                    console.log(
                        `${IN}${player.name} is affected by ${event.name}`
                    );
                    console.log(
                        `${IN}		pay item || pay 2 resources & move to HKC`
                    );
...
function announceGameStart() {
    console.log(
        `${A_PREFIX}It's 2047 and Hong Kong is in the midst of ever-growing inequality.`,
        style
    );
    console.log(`%c${IN}${SYMTACTICS.winCondition}`, style);
    console.log(
        `%c${IN}Let the simulation game of future tactical urbanism begin.`,
        style
    );
}
...
function announceDroneReassignment(players) {
    players.forEach(function (player) {
        console.log(
            `${A_PREFIX}drone is with ${player.name} (${player.id}), ${player.drone}`,
            style
        );
    });
}
    ...
    hexes.forEach(function (hex) {
        if (hex.players.length > 0 && hex.site) {
            hex.players.forEach(function (player) {
                console.log(
                    `${IN}${player.name} (${player.id}) is in		${hex.zone.name}	at		${hex.site.name}`
                );
            });
